...
/// Halaman "Check Your Email" setelah register atau saat perlu resend verification.
/// Menampilkan countdown 5 menit untuk resend verification token.
class EmailSentScreen extends StatefulWidget {
  final String email;
  /// Apakah datang dari login (true) atau dari register (false)
...
class _EmailSentScreenState extends State<EmailSentScreen>
  ...
  // Animasi untuk ikon email
  late AnimationController _envelopeController;
  late Animation<double> _envelopeBounce;
  late Animation<double> _envelopeGlow;

  // Animasi untuk pulse effect pada timer
  ...
  void initState() {
    ...
    _envelopeBounce = Tween<double>(begin: 0, end: -10).animate(
      CurvedAnimation(
        parent: _envelopeController,
        curve: Curves.easeInOut,
      ),
    );
    _envelopeGlow = Tween<double>(begin: 0.3, end: 0.7).animate(
      CurvedAnimation(
        parent: _envelopeController,
        curve: Curves.easeInOut,
      ),
    ...
    // Jika datang dari register, langsung mulai timer (email sudah terkirim oleh BE saat register)
    if (!widget.fromLogin) {
      _startCooldown();
    }
  }
  ...
  Future<void> _handleResend() async {
    ...
    if (result['success'] == true) {
      ToastHelper.showSuccess(
          context, result['message'] ?? 'Email verifikasi berhasil dikirim ulang!');
      _startCooldown();
    } else {
      ToastHelper.showError(
          context, result['message'] ?? 'Gagal mengirim ulang email verifikasi.');
    }
  }
                  ...
                  children: [
                              ...
                              boxShadow: [
                                BoxShadow(
                                  color: AppColors.primaryAction
                                      .withOpacity(_envelopeGlow.value * 0.3),
                                  blurRadius: 30,
                                  spreadRadius: 5,
                                ),
                              ],
                    ...
                    RichText(
                      ...
                      text: TextSpan(
                        ...
                        children: [
                          TextSpan(
                            text: widget.fromLogin
                                ? 'Akun kamu sudah terdaftar tetapi email belum diverifikasi. '
                                  'Silakan kirim ulang email verifikasi ke '
                    ...
                    ClipRRect(
                      ...
                      child: BackdropFilter(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: [
                              // Timer countdown (shown when cooling down)
                              if (!_canResend) ...[
